...
class BayesCalibrationMCMC:
    def __init__(self, data: list, data_time_indices: list, metamodels: list, distribution: object, index: int, scalers: list) -> None:
        self.data = data
    ...
    class CalibrationFunction:
        def __init__(self, metamodel: ot.metamodel, data:list, data_time_indices: list, index: int, scaler:object
        ) -> None:
        ...
        def __call__(self, theta: list, x: list, index: int) -> np.array:
            ...
            a[index] = theta[0]
            x = a
            return self.metamodel.predict(self.scaler.transform(np.asarray(x).reshape(1,-1)))[0, time_indices]
            
    class LikelihoodFunction:
        def __init__(self, data: np.array, data_time_indices: list, calibration_functions: list, nb_weights: int, nb_points: int, distribution: object, index: int
                     ) -> None:
            self.data = data
        ...
        def __call__(self) -> ot.PythonFunction:
            ...
            def log_likelihood(theta) -> float:
                ...
                #sample from the prior distribution for mc integration
                other_params = np.asarray(self.distribution.getSample(self.nb_points))
                
                log_likelihoods = []
                for k in range(self.nb_points):
                    cal_funcs = ((np.asarray([np.asarray((self.calibration_functions)[i](theta, other_params[k, :], self.index)) for i in range(len(self.calibration_functions))])).T)#[0,:]
                    ...
                    for i in range(self.nb_weights):
                        ...
                        log_likelihoods_k.append(log_pdf)
                    log_likelihoods.append(np.asarray(log_likelihoods_k))
                max_logs = np.max(np.asarray(log_likelihoods))

                # Compute the log-likelihood using logsumexp trick
                log_pdf = max_logs + np.log(np.sum(np.exp(np.asarray(log_likelihoods)) - max_logs)) - np.log(self.nb_weights) - np.log(self.nb_points)
                print(log_pdf)
    ...
    def mcmc_run(
        ...
        proposal: ot.Distribution, burning: int, n_chains: int, sample_size: int, distribution: object, index: int, nb_weights: int, nb_points: int
    ) -> list:
        ...
        calibration_functions = [self.CalibrationFunction((self.metamodels)[i], self.data, self.data_time_indices, self.index, (self.scalers)[i]) for i in range(len(self.metamodels))]
        # Initialize likelihood function
        likelihood_func = self.LikelihoodFunction(self.data, self.data_time_indices, calibration_functions=calibration_functions, nb_weights=self.nb_weights, nb_points=self.nb_points, distribution=self.distribution, index=self.index)()
